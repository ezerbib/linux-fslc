						...
						struct i2c_msg *msgs, int num)
{
	...
	if (result) {
		...
		if (i2c_imx->adapter.bus_recovery_info && (i2c_imx->recovery_cnt<10) ) {
			...
			result = i2c_imx_start(i2c_imx);
			dev_dbg(&i2c_imx->adapter.dev, "<%s> after recover result=%d\n", __func__,result);
		}
	}
		...
		struct platform_device *pdev)
{
	...
	/*dev_err(&pdev->dev, "using scl-gpio %d and sda-gpio %d for recovery\n",
	...
	if (rinfo->sda_gpio == -EPROBE_DEFER ||
		...
		if (!gpio_is_valid(rinfo->sda_gpio))
			dev_dbg(&pdev->dev, "!gpio_is_valid(rinfo->sda_gpio)\n");
		if (!gpio_is_valid(rinfo->scl_gpio))
			dev_dbg(&pdev->dev, "!gpio_is_valid(rinfo->scl_gpio)\n");
		if (IS_ERR(i2c_imx->pinctrl_pins_default))
			dev_dbg(&pdev->dev, "IS_ERR(i2c_imx->pinctrl_pins_default)\n");
		if (IS_ERR(i2c_imx->pinctrl_pins_gpio))
			dev_dbg(&pdev->dev, "IS_ERR(i2c_imx->pinctrl_pins_gpio)\n");
		return 0;
	}
